...
<div class="mx-auto space-y-8 animate-fadeIn">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 class="text-2xl font-semibold text-slate-900">Crear Nuevo Rubro</h2>
        <a href="{{ route('rubros.index') }}" class="inline-flex items-center px-4 py-2 border border-slate-300 text-sm font-medium rounded-md text-slate-700 hover:bg-slate-50 transition duration-150 cursor-pointer">
            ← Volver a la lista
        </a>
    </div>
    ...
    <form id="rubroForm" class="bg-white rounded-md border border-slate-200 p-6 space-y-6 shadow-sm">
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            <button type="submit"
                ...
                <span id="submitText">Crear Rubro</span>
        ...
        .then(data => {
            if (data.success) {
                ...
                if (data.errors) {
                    ...
                    showAlert(data.message || 'Error al crear el rubro', 'error');
                }
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showAlert('Error al crear el rubro', 'error');
        })
        .finally(() => {
