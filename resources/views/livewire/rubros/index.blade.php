...
<div class="space-y-8 animate-fadeIn bg-white border border-slate-200 rounded-md shadow-sm p-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 class="text-2xl font-semibold text-slate-900">Rubros (<span id="totalCount">{{ $rubros->total() }}</span>)</h2>
    ...
    <p class="text-sm text-slate-500 text-center">
        Página <span id="currentPage">{{ $rubros->currentPage() }}</span> de <span id="lastPage">{{ $rubros->lastPage() }}</span> -
        Mostrando <span id="firstItem">{{ $rubros->firstItem() }}</span>-<span id="lastItem">{{ $rubros->lastItem() }}</span> de <span id="totalItems">{{ $rubros->total() }}</span> registros
    </p>
        ...
        <table class="w-full text-sm text-slate-700">
            <thead class="bg-slate-50 text-slate-600 text-xs uppercase">
                <tr>
                    ...
                    <th class="px-4 py-3 text-center font-medium">Categorías</th>
                    <th class="px-4 py-3 text-center font-medium">Acciones</th>
                </tr>
            </thead>
            <tbody id="rubrosTable" class="divide-y divide-slate-200">
                @include('livewire.rubros.partials.table', ['rubros' => $rubros])
            </tbody>
        </table>
    </div>

    <div id="pagination">
        @include('livewire.rubros.partials.pagination', ['rubros' => $rubros])
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        searchTimeout = setTimeout(() => loadRubros(), 300);
    });
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.delete-btn')) {
            ...
            if (confirm('¿Estás seguro de eliminar este rubro? Las categorías asociadas quedarán sin rubro.')) {
                fetch(`/admin/rubros/${id}`, {
                    method: 'DELETE',
                    headers: {
                ...
                .catch(error => {
                    ...
                    showAlert('Error al eliminar el rubro', 'error');
                });
            }
        }
    });
    ...
    function loadRubros(page = 1) {
        ...
        const url = `{{ route('rubros.index') }}?${params.toString()}`;
        ...
        .then(response => response.json())
        .then(data => {
            document.getElementById('rubrosTable').innerHTML = data.html;
        ...
        .catch(error => {
            console.error('Error:', error);
            showAlert('Error al cargar los rubros', 'error');
        });
    }
