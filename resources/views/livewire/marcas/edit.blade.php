...
<div class="max-w-4xl mx-auto space-y-8 animate-fadeIn">
    ...
    <form action="{{ route('marcas.update', $marca) }}" method="POST" class="bg-white border border-slate-200 rounded-md shadow-sm p-8 space-y-6">
        @csrf
        @method('PUT')
        
        <input type="hidden" name="deleted_modelos[]" id="deletedModelos">
        ...
        <div class="space-y-6">
            ...
            <div id="modelosContainer" class="space-y-4">
                @foreach($marca->modelos as $index => $modelo)
                    <div class="modelo-item p-4 border border-slate-200 rounded-md bg-slate-50 space-y-4" data-index="{{ $index }}" data-id="{{ $modelo->id }}">
                        <div class="flex items-center justify-between">
                            <h4 class="font-medium text-slate-700">Modelo #{{ $index + 1 }}</h4>
                            <button type="button" 
                                    onclick="removeModelo({{ $index }}, {{ $modelo->id }})"
                        ...
                        <input type="hidden" name="modelos[{{ $index }}][id]" value="{{ $modelo->id }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" 
                                       ...
                                       value="{{ old('modelos.'.$index.'.title', $modelo->title) }}"
                            ...
                            <div>
                                ...
                                <input type="text" 
                                       ...
                                       value="{{ old('modelos.'.$index.'.order', $modelo->order) }}"
                                       class="w-full px-4 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-600">
                            </div>
                        </div>
                    </div>
...
<script>
let modeloIndex = {{ $marca->modelos->count() }};
let deletedModelos = [];
...
function updateDeletedInput() {
    const container = document.getElementById('deletedModelos').parentElement;
    container.querySelectorAll('input[name="deleted_modelos[]"]').forEach(input => input.remove());
    
    deletedModelos.forEach(id => {
        const input = document.createElement('input');
        input.type = 'hidden';
        input.name = 'deleted_modelos[]';
        input.value = id;
