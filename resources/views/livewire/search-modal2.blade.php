<div x-data="{ isOpen: @entangle('isOpen') }" @click.outside="isOpen && $wire.toggleSearch()">
    <button wire:click="toggleSearch" 
        ...
        <svg class="w-6 h-6 text-black group-hover:text-[#BA2025] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
        </svg>
    </button>

    <div class="hidden lg:block absolute top-[100px] left-0 right-0 bg-white  transform transition-all duration-300 ease-in-out z-[60] border-b border-gray-200"
         x-show="isOpen"
            ...
            <div class="overflow-y-auto max-h-[50vh] custom-scroll">
                @if($this->search && strlen($this->search) >= 2)
                    ...
                    @elseif($this->results->isEmpty())
                        <p class="text-center py-12 text-gray-500 text-lg">No encontramos productos para "{{ $this->search }}"</p>
                    @else
    ...
    <div x-show="isOpen" 
         class="lg:hidden fixed inset-0 z-[100] bg-white"
         x-transition:enter="transition ease-out duration-300"
