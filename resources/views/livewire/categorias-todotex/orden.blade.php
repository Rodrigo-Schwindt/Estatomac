...
<div class="space-y-6 animate-fadeIn">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-semibold text-slate-900">Orden de productos — {{ $categoria->titulo }}</h2>
            @if($categoria->familia)
                <p class="text-sm text-slate-400 mt-0.5">{{ $categoria->familia->titulo }}</p>
            @endif
            <p class="text-sm text-slate-500 mt-1">Asigná un número a cada producto para definir su orden dentro de esta categoría. Sin número → al final.</p>
        </div>
        ...
        <div class="bg-white border border-slate-200 rounded-md shadow-sm overflow-hidden">
            ...
            <table class="w-full text-sm text-slate-700">
                <thead class="bg-slate-50 text-slate-500 text-xs uppercase border-b border-slate-200">
                    <tr>
                        <th class="px-4 py-3 text-center font-medium w-28">Orden</th>
            ...
            <div class="px-5 py-3 border-t border-slate-100 flex items-center justify-center gap-1 flex-wrap">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    guardarBtn.addEventListener('click', function () {
        ...
        const inputs  = document.querySelectorAll('#ordenForm input[name^="ordenes["]');
        const ordenes = {};
        ...
        fetch('{{ route("categorias-todotex.guardarOrden", $categoria->id) }}', {
            method: 'POST',
            headers: {
        ...
        .then(r => r.json())
        .then(data => {
            showAlert(data.success ? data.message : 'Error al guardar el orden.', data.success ? 'success' : 'error');
        })
