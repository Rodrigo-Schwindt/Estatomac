...
<div class="mx-auto space-y-8 animate-fadeIn">
    ...
    <form id="familiaForm" class="bg-white rounded-md border border-slate-200 p-6 space-y-6 shadow-sm">
        @csrf
        ...
        <div class="flex gap-6">
            {{-- <div class="flex items-center gap-3">
                <input type="checkbox"
                       ...
                       name="destacado"
                       value="1"
                       {{ $familia->destacado ? 'checked' : '' }}
                       class="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-2 focus:ring-blue-600">
                <label for="destacado" class="text-sm font-medium text-slate-900 cursor-pointer">Destacado</label>
            </div> --}}
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox"
                       ...
                       name="visible"
                       value="1"
                       {{ $familia->visible ? 'checked' : '' }}
                       class="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-2 focus:ring-blue-600">
                <label for="visible" class="text-sm font-medium text-slate-900 cursor-pointer">Visible</label>
            </div>
        </div>

        <div x-data="{ preview: null, existing: '{{ $familia->imagen ? Storage::url($familia->imagen) : '' }}' }">
            ...
            <div class="flex items-start gap-4">
                <label class="cursor-pointer flex flex-col items-center justify-center w-40 h-28 border-2 border-dashed border-slate-300 rounded-lg hover:border-blue-500 transition-colors bg-slate-50">
                    ...
                    <span class="text-xs text-slate-500">{{ $familia->imagen ? 'Cambiar imagen' : 'Subir imagen' }}</span>
                    <input type="file" name="imagen" accept="image/*" class="hidden"
                           @change="preview = $event.target.files[0] ? URL.createObjectURL($event.target.files[0]) : null">
                </label>
                <div x-show="preview || existing" class="relative">
                    <img :src="preview || existing" alt="Banner de familia" class="w-40 h-28 object-cover rounded-lg border border-slate-200">
                    @if($familia->imagen)
            ...
            <p class="text-xs text-slate-500 mt-1">JPG, PNG, WEBP · Máx. 3 MB. Se muestra como banner al filtrar por esta familia.</p>
        </div>
        ...
        <div class="flex justify-end space-x-3 pt-4 border-t border-slate-200">
            ...
            <button type="submit"
                ...
                <span id="submitText">Actualizar Familia</span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        fetch(`{{ route('familias.update', $familia->id) }}`, {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                if (data.errors) {
                    ...
                    showAlert(data.message || 'Error al actualizar la familia', 'error');
                }
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showAlert('Error al actualizar la familia', 'error');
        })
        .finally(() => {
    ...
    if (btnDeleteImagen) {
        btnDeleteImagen.addEventListener('click', function() {
            if (!confirm('¿Eliminar la imagen de esta familia?')) return;
            fetch(`{{ route('familias.deleteImagen', $familia->id) }}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    'Accept': 'application/json',
