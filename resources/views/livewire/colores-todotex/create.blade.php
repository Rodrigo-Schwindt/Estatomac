...
<div class="mx-auto space-y-8 animate-fadeIn">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 class="text-2xl font-semibold text-slate-900">Crear Nuevo Color</h2>
        <a href="{{ route('colores-todotex.index') }}" class="inline-flex items-center px-4 py-2 border border-slate-300 text-sm font-medium rounded-md text-slate-700 hover:bg-slate-50 transition duration-150 cursor-pointer">
            ← Volver a la lista
        </a>
    </div>
    ...
    <form id="colorForm" class="bg-white rounded-md border border-slate-200 p-6 space-y-6 shadow-sm">
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-1">Debe tener exactamente 4 caracteres.</p>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            <button type="submit"
                ...
                <span id="submitText">Crear Color</span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        fetch('{{ route("colores-todotex.store") }}', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                showAlert(data.message || 'Error al crear el color', 'error');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showAlert('Error al crear el color', 'error');
        })
        .finally(() => {
