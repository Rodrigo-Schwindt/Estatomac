    ...
    <nav class="hidden lg:flex items-center">
        <div class="flex h-[40px] py-2 gap-[17px] items-center">
            <a wire:navigate href="{{ route('cliente.productos') }}"
               class="nav-link {{ request()->is('zona-privada/productos') ? 'active' : '' }} text-black font-inter text-[14px] font-normal leading-normal relative">
               PRODUCTOS
            </a>
    
            <a wire:navigate href="{{ route('cliente.carrito') }}"
            class="nav-link {{ request()->is('zona-privada/carrito') ? 'active' : '' }} text-black font-inter text-[14px] font-normal leading-normal relative">
                CARRITO
            </a>
            
            <a wire:navigate href="{{ route('cliente.informar-pago') }}"
            class="nav-link {{ request()->routeIs('cliente.informar-pago') ? 'active' : '' }} text-black font-inter text-[14px] font-normal leading-normal relative">
             INFORMACIÓN DE PAGOS
         </a>
         <a wire:navigate href="{{ route('cliente.precios') }}"
         class="nav-link {{ request()->is('zona-privada/precios') ? 'active' : '' }} text-black font-inter text-[14px] font-normal leading-normal relative">
             LISTA DE PRECIOS
         </a>
         <a wire:navigate href="{{ route('cliente.pedidos') }}"
         class="nav-link {{ request()->is('zona-privada/pedidos') ? 'active' : '' }} text-black font-inter text-[14px] font-normal leading-normal relative">
             MIS PEDIDOS
         </a>
         @auth('cliente')
