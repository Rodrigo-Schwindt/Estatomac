...
<div class="max-w-[1224px] mx-auto flex justify-between items-center w-full px-4 lg:px-0">
    ...
    <a wire:navigate href="{{ route('cliente.productos') }}" class="w-fit h-[58px] flex-shrink-0 cursor-pointer max-lg:w-40">
    ...
    <nav class="hidden lg:flex items-center">
        <div class="flex h-[40px] py-2 gap-[17px] items-center">
            ...
            <a wire:navigate href="{{ route('cliente.precios') }}"
        ...
        <a wire:navigate href="{{ route('cliente.informar-pago') }}"
        class="nav-link {{ request()->routeIs('cliente.informar-pago') ? 'active' : '' }} text-black font-inter text-[16px] font-normal leading-normal relative">
         Info de pagos
     </a>
...
@php
    ...
    if (Auth::guard('cliente')->check()) {
        $zoneUser = Auth::guard('cliente')->user();
        $zoneRole = 'cliente';
    } elseif (Auth::guard('vendedor')->check()) {
        $zoneUser = Auth::guard('vendedor')->user();
        $zoneRole = 'vendedor';
    }
@endphp
...
<div x-data="{ open: false }" @click.away="open = false" class="relative">
    <button
        @click="open = !open"
        class="nav-link text-white cursor-pointer bg-[#23378C] font-inter text-[16px] font-normal
               w-[155px] h-[40px] rounded-[40px] border border-[#23378C]
               flex justify-center items-center text-center gap-2
               hover:bg-black hover:text-white transition-colors">
        <span class="truncate max-w-[110px]">{{ $zoneUser->nombre }}</span>
        <svg :class="{ 'rotate-180': open }" class="w-4 h-4 flex-shrink-0 transition-transform duration-200"
    ...
    <div x-show="open"
         ...
         class="absolute right-0 mt-2 w-[220px] bg-white rounded-lg shadow-xl border border-gray-100 py-2 z-50"
         style="display:none;">

        <div class="px-4 py-2 border-b border-gray-100">
            <p class="text-[13px] font-semibold text-gray-900 truncate">{{ $zoneUser->nombre }}</p>
            @if($zoneUser->email)
                <p class="text-[11px] text-gray-500 truncate">{{ $zoneUser->email }}</p>
            @endif
            <span class="inline-block mt-1 text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded-full
                {{ $zoneRole === 'vendedor' ? 'bg-[#23378C]/10 text-[#23378C]' : 'bg-green-100 text-green-700' }}">
                {{ $zoneRole === 'vendedor' ? 'Vendedor' : 'Cliente' }}
            </span>
        </div>

        <a wire:navigate href="{{ route('cliente.pedidos') }}" @click="open = false"
           class="flex items-center gap-3 px-4 py-2.5 text-gray-700 hover:bg-gray-50 transition-colors">
            ...
            <span class="text-[14px]">{{ $zoneRole === 'vendedor' ? 'Mis pedidos' : 'Mis pedidos' }}</span>
        </a>

        @if($zoneRole === 'cliente')
        <a @click="open = false; $dispatch('open-editar-datos')"
        ...
        <div class="border-t border-gray-100 mt-1">
            @if($zoneRole === 'vendedor')
            <form method="POST" action="{{ route('cliente.vendedor.logout') }}">
                @csrf
                <button type="submit"
                        class="w-full cursor-pointer flex items-center gap-3 px-4 py-2.5 text-red-600 hover:bg-red-50 transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<aside 
    ...
    <nav class="px-6 py-6 space-y-2">
        <a wire:navigate 
           ...
           class="group flex items-center gap-3 px-4 py-3 rounded-lg transition-all
            ...
            <svg class="w-5 h-5 {{ request()->is('zona-privada/productos') ? '' : 'text-gray-400 group-hover:text-[#BA2025]' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a wire:navigate 
           ...
           class="group flex items-center gap-3 px-4 py-3 rounded-lg transition-all
                  {{ request()->is('zona-privada/carrito') 
                      ? 'bg-[#BA2025] text-white shadow-md' 
                      : 'text-gray-700 hover:bg-gray-50 hover:text-[#BA2025]' }}">
            <svg class="w-5 h-5 {{ request()->is('zona-privada/carrito') ? '' : 'text-gray-400 group-hover:text-[#BA2025]' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a wire:navigate 
           href="{{ route('cliente.informar-pago') }}"
           @click="open = false"
           class="group flex items-center gap-3 px-4 py-3 rounded-lg transition-all
                  {{ request()->routeIs('cliente.informar-pago') 
                      ? 'bg-[#BA2025] text-white shadow-md' 
                      : 'text-gray-700 hover:bg-gray-50 hover:text-[#BA2025]' }}">
            <svg class="w-5 h-5 {{ request()->routeIs('cliente.informar-pago') ? '' : 'text-gray-400 group-hover:text-[#BA2025]' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/>
            </svg>
            <span class="font-montserrat text-[16px] font-medium">Información de Pagos</span>
        </a>

        <a wire:navigate 
           href="{{ route('cliente.precios') }}"
           @click="open = false"
           class="group flex items-center gap-3 px-4 py-3 rounded-lg transition-all
                  {{ request()->is('zona-privada/precios') 
                      ? 'bg-[#BA2025] text-white shadow-md' 
                      : 'text-gray-700 hover:bg-gray-50 hover:text-[#BA2025]' }}">
            <svg class="w-5 h-5 {{ request()->is('zona-privada/precios') ? '' : 'text-gray-400 group-hover:text-[#BA2025]' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a wire:navigate 
           ...
           class="group flex items-center gap-3 px-4 py-3 rounded-lg transition-all
                  {{ request()->is('zona-privada/pedidos') 
                      ? 'bg-[#BA2025] text-white shadow-md' 
                      : 'text-gray-700 hover:bg-gray-50 hover:text-[#BA2025]' }}">
            <svg class="w-5 h-5 {{ request()->is('zona-privada/pedidos') ? '' : 'text-gray-400 group-hover:text-[#BA2025]' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
            </svg>
    ...
    class="group flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-gray-700 hover:bg-gray-50 hover:text-[#BA2025] cursor-pointer"
>
    <svg class="w-5 h-5 text-gray-400 group-hover:text-[#BA2025]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
    </svg>
    <span class="font-montserrat text-[16px] font-medium">Mis datos</span>
        ...
        <div class="pt-4 border-t border-gray-200">
            <div class="px-4 py-3 bg-gray-50 rounded-lg mb-2">
                <p class="text-[13px] font-semibold text-gray-900 truncate">{{ $zoneUser->nombre }}</p>
                @if($zoneUser->email)
                    <p class="text-[11px] text-gray-500 break-all">{{ $zoneUser->email }}</p>
                @endif
                <span class="inline-block mt-1 text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded-full
                    {{ $zoneRole === 'vendedor' ? 'bg-[#23378C]/10 text-[#23378C]' : 'bg-green-100 text-green-700' }}">
                    {{ $zoneRole === 'vendedor' ? 'Vendedor' : 'Cliente' }}
